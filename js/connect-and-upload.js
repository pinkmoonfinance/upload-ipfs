function updateNode(selectedNode) {
  if (selectedNode == "remote") {
    ...
    node.remote.gateway = document
      .querySelector("#remote")
      .querySelector("#remote_gatewayPort").value;
    node.remote.protocol = document
      .querySelector("#remoteProtocol")
  ...
  if (selectedNode == "local") {
    ...
    node.local.gateway = document
      .querySelector("#local")
      .querySelector("#local_gatewayPort").value;
    node.local.protocol = document
      .querySelector("#localProtocol")
      .querySelector("li.active")
      .innerText.toLowerCase();
  }
...
function nodeConnect(selectedNode) {
  if (selectedNode == "remote") {
    ...
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent div").innerText = "Connecting";
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent div")
      .classList.add("connecting");
    document
      .querySelector("button#buttonLocal")
      .querySelector(".buttonContent div").innerHTML =
      'Connect <img src="img/connect.png"/>';
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent .min-loading")
      .classList.remove("min-loading-hidden"); //loading event
  }
  ...
  ipfsRequest("GatewayCheck.log", buffer.Buffer.from("ABC", "utf-8")).then(
    (data) => {
      ...
      document
        .querySelector("button#buttonRemote")
        .querySelector(".buttonContent .min-loading")
        .classList.add("min-loading-hidden"); //loading event
      document
        .querySelector("button#buttonLocal")
        .querySelector(".buttonContent .min-loading")
        .classList.add("min-loading-hidden"); //loading event
...
function online(selectedNode) {
  ...
  if (selectedNode == "remote") {
    ...
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent div").innerHTML =
      'Node Online <img src="img/connected.png"/>';
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent .min-loading")
      .classList.add("min-loading-hidden");
  }
  if (selectedNode == "local") {
    document
      .querySelector("button#buttonLocal")
...
function offline(selectedNode) {
  ...
  if (selectedNode == "remote") {
    document
      .querySelector("button#buttonRemote")
      .querySelector(".buttonContent div").innerHTML =
      'Node Offline! <img src="img/disconnected.png"/>';
  }
  if (selectedNode == "local") {
    document
      .querySelector("button#buttonLocal")
...
function upload() {
  ...
  document
    .querySelector(".min-loading.blue")
    .classList.remove("loading-hidden"); //loading event
    ...
    reader.onloadend = function () {
      ipfsRequest(file.name, buffer.Buffer(reader.result)).then((data) => {
        ...
        if (uploadCount == filesOk.length) {
          document
            .querySelector(".min-loading.blue")
            .classList.add("loading-hidden"); //stop loading event
          document
            .querySelector("button#buttonRemote")
          ...
          document
            .querySelector("button#buttonLocal")
            .removeAttribute("disabled", "");
          document.querySelector("button#buttonUpload").onclick = function () {
            resetFiles();
          };
          document.querySelector("button#buttonUpload").innerHTML =
            'Clean Up<img src="img/reset.png" />';
          document
...
function handleFileSelect(evt) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if (files[i].size == 0) {
      alert(
        "Skipping " + files[i].name.toUpperCase() + " because it is empty."
      );
      continue;
    }
    if (files_checksum.includes(fileChecksum(files[i]))) {
      ...
      document.querySelector("#list").querySelector("ul").innerHTML +=
        '<li id="' +
        fileChecksum(files[i]) +
        '"><strong class="fileName">' +
        files[i].name +
        '</strong> <spam class="itemClose"><a class="removeItem" href="#" onclick="removeItem(\'' +
        fileChecksum(files[i]) +
        "')\">&times;</a></spam>" +
        '</a></spam><br> <spam id="fileProperties"> (' +
        (files[i].type || "n/a") +
        ") - " +
        files[i].size +
        " bytes, last modified: " +
        new Date(files[i].lastModified).toLocaleDateString() +
        "</spam></li>";
...
function updateList(checksum, ipfsHash) {
  ...
  if (
    node[node.default].gateway == 80 ||
    node[node.default].protocol == "https"
  ) {
    var gatewayPort = "";
  } else {
...
function resetFiles() {
  ...
  uploadCount = 0;
  document.querySelector("#list").querySelector("ul").innerHTML = "";
  document.querySelector("pre#response").innerHTML =
    '<spam id="info">Response IPFS API:</spam>';
  document.querySelector("button#buttonUpload").onclick = function () {
    upload();
  };
  document.querySelector("button#buttonUpload").innerHTML =
    'Upload<img src="img/upload.png">';
}
